...
import { estimateTravelTime, formatDistance, skillOverlapScore } from '../utils/geo';
    ...
    <Badge variant={variant} className="gap-1 text-[10px] sm:text-xs">
      <svg className="h-3 w-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path
          ...
          strokeWidth="3"
          d={overlap >= 40 ? 'M9 12l2 2 4-4' : 'M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z'}
        />
      </svg>
      {overlap}% match
    </Badge>
...
export const GigCard = memo(function GigCard({ gig, volunteerSkills = [] }: GigCardProps) {
  ...
  return (
    <article className="group relative flex flex-col rounded-2xl border border-emerald-50/50 bg-white p-5 shadow-xs hover:-translate-y-1 hover:shadow-md hover:shadow-emerald-950/3 hover:border-emerald-100/80 dark:border-slate-700 dark:bg-slate-800 dark:shadow-none dark:shadow-slate-900/50 dark:hover:border-slate-600 dark:hover:shadow-none dark:hover:shadow-slate-900/50 transition-all duration-300 ease-out overflow-hidden">
      ...
      {isFeatured && (
        <div className="absolute top-3 right-0 z-10">
          <div className="relative">
            <svg
              className="h-7 w-7 text-amber-400 drop-shadow-sm"
              viewBox="0 0 24 24"
              fill="currentColor"
            >
              <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
            </svg>
            <span className="absolute inset-0 flex items-center justify-center text-[7px] font-black text-white pt-0.5">
              FEATURED
            </span>
          </div>
        </div>
      ...
      <div
        className={`absolute top-0 left-0 h-[3px] w-full ${isFeatured ? 'bg-gradient-to-r from-amber-400 to-yellow-300' : overlap >= 75 ? 'bg-gradient-to-r from-emerald-500 to-teal-400' : overlap >= 40 ? 'bg-gradient-to-r from-amber-400 to-orange-300' : 'bg-gradient-to-r from-slate-200 to-slate-300 dark:from-slate-600 dark:to-slate-500'}`}
      />
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 flex flex-col items-end gap-1">
          <Badge variant="secondary" className="text-[10px] sm:text-xs px-2.5 py-0.5">
      ...
      {/* NGO name */}
      <p className="mt-1.5 text-xs sm:text-sm font-black text-emerald-700 dark:text-emerald-400 opacity-90">
        {gig.ngo_name}
      </p>
          ...
          {gig.required_skills.map((skill) => {
            ...
            return (
              <Badge
                ...
                className="gap-0.5 text-[10px] sm:text-xs px-2.5 py-0.5"
              >
                {hasSkill && (
                  <svg
