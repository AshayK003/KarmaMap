          ...
          <div className="max-h-80 overflow-y-auto">
            {notifications.length === 0 ? (
              <div className="flex flex-col items-center py-8 text-center px-4">
                <svg
                  className="h-8 w-8 text-slate-300 dark:text-slate-600 mb-2"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                ...
                <p className="text-[11px] text-slate-300 dark:text-slate-600 mt-0.5">
                  Match alerts and updates will appear here.
                </p>
              </div>
            ) : (
              notifications.map((n) => (
                <div
                  ...
                  aria-label={
                    n.read_status ? 'Notification' : 'Unread notification — click to mark as read'
                  }
                >
                  <div
                    className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${!n.read_status ? 'bg-emerald-500' : 'bg-transparent'}`}
                  />
