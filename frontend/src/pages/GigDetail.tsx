...
export function GigDetail() {
  ...
  const getWeatherAdvisory = () => {
    ...
    if (code >= 95 || [63, 65, 81, 82].includes(code)) {
      return {
        emoji: '⛈️',
        bg: 'bg-rose-50/70 border-rose-200/50 text-rose-800 shadow-rose-950/2',
    ...
    if ([51, 53, 55, 61, 80].includes(code)) {
      return {
        emoji: '☔',
        bg: 'bg-blue-50/70 border-blue-200/50 text-blue-800 shadow-blue-950/2',
    ...
    if (tempMax > 35) {
      return {
        emoji: '☀️',
        bg: 'bg-amber-50/70 border-amber-200/50 text-amber-900 shadow-amber-950/2',
    ...
    if (tempMax < 8) {
      return {
        emoji: '❄️',
        bg: 'bg-sky-50/70 border-sky-200/50 text-sky-900 shadow-sky-950/2',
    ...
    if (code === 0 || code === 1) {
      return {
        emoji: '✨',
        bg: 'bg-emerald-50/70 border-emerald-200/50 text-emerald-800 shadow-emerald-950/2',
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <button
        onClick={() => navigate(-1)}
        className="mb-5 inline-flex items-center gap-1 text-xs font-black text-slate-400 hover:text-emerald-600 transition-colors cursor-pointer"
      >
      ...
      <h1 className="text-3xl font-black tracking-tight text-slate-800 leading-tight">{gig.title}</h1>
      <p className="mt-1 text-sm font-bold text-emerald-700">
        by {(gig as Gig & { profiles?: { name: string } }).profiles?.name ?? 'NGO'}
      </p>
      
      <p className="mt-5 text-sm font-semibold leading-relaxed text-slate-600 bg-white border border-slate-100 rounded-2xl p-5 shadow-2xs whitespace-pre-wrap">
        {gig.description}
      </p>

      {/* Skills requirements */}
      <div className="mt-5">
        <span className="text-[10px] font-extrabold uppercase tracking-widest text-gray-400 block mb-2">Required Skills</span>
        ...
        <Card className="p-5 flex flex-col justify-between">
          <div className="space-y-2">
            ...
            <div className="flex items-baseline gap-1.5">
              <span className="text-3xl font-black text-slate-800">{volunteerCount || gig.volunteers_joined}</span>
          ...
          {profile?.role === 'volunteer' && (
            <div className="mt-4 pt-3 border-t border-slate-50 flex items-center justify-between">
              <span className="text-xs font-bold text-slate-500">Profile Match</span>
            ...
            {loadingWeather ? (
              ...
              <div className="flex items-center gap-3 w-full justify-between">
                <div className="flex items-center gap-2.5">
                  <div className="bg-slate-50/50 border border-slate-100 p-1 rounded-xl">
