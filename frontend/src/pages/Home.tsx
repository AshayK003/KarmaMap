    ...
    <div className="relative min-h-[calc(100vh-64px)] overflow-hidden bg-gradient-to-b from-emerald-50/20 via-slate-50 to-white dark:from-slate-900 dark:via-slate-900 dark:to-slate-900">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20 sm:py-28 relative z-10">
        ...
        <section className="mt-20 border-y border-slate-100 dark:border-slate-700 py-10 bg-white/40 dark:bg-slate-800/40 backdrop-blur-xs rounded-3xl px-6 grid gap-8 sm:grid-cols-3 text-center">
          <div>
            <p className="text-3xl sm:text-4xl font-black text-slate-800 dark:text-slate-100">{stats.totalHours.toLocaleString()}+</p>
          ...
          <div className="border-y sm:border-y-0 sm:border-x border-slate-100 dark:border-slate-700 py-6 sm:py-0">
            <p className="text-3xl sm:text-4xl font-black text-emerald-600 dark:text-emerald-400">{stats.openGigs}+</p>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Open Opportunities</p>
          </div>
          <div>
            <p className="text-3xl sm:text-4xl font-black text-slate-800 dark:text-slate-100">{stats.ngoCount}+</p>
        ...
        <section className="mt-20">
          <div className="text-center max-w-xl mx-auto mb-12">
            <h2 className="text-2xl font-black text-slate-800 dark:text-slate-100 sm:text-3xl">Engineered for Community Impact</h2>
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {[
              ...
              <Card key={f.title} className="group p-6 hover:shadow-md dark:shadow-none dark:shadow-slate-900/50 hover:border-emerald-100/50 transition-all duration-200">
                ...
                <h3 className="mt-4 text-base font-extrabold text-slate-800 dark:text-slate-100">{f.title}</h3>
                <p className="mt-2 text-xs font-semibold text-slate-500 dark:text-slate-400 leading-normal">{f.desc}</p>
              </Card>
            ))}
          </div>
        </section>
