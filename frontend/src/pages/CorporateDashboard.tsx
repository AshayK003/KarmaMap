...
import { Building2Icon, AwardIcon, UsersIcon, ClockIcon } from '../components/NavIcons';
    ...
    return (
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
          ...
          <h2 className="text-xl font-black text-slate-700 dark:text-slate-200 mb-2">Dashboard Unavailable</h2>
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-6 lg:grid-cols-2 mb-8">
        <Card>
          <h2 className="text-lg font-black text-slate-800 dark:text-slate-100 mb-1">Hours by Department</h2>
          <p className="text-xs font-semibold text-slate-400 mb-4">Breakdown of volunteer hours across departments</p>
          {analytics.hours_by_department.length === 0 ? (
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={analytics.hours_by_department} margin={{ top: 8, right: 8, left: 0, bottom: 8 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
                  <XAxis dataKey="department" tick={{ fontSize: 10, fill: '#64748b', fontWeight: 500 }} tickLine={false} axisLine={false} dy={6} />
                  <YAxis tick={{ fontSize: 10, fill: '#64748b', fontWeight: 500 }} tickLine={false} axisLine={false} dx={-4} width={32} />
        ...
        <Card>
          <h2 className="text-lg font-black text-slate-800 dark:text-slate-100 mb-1">Hours Over Time</h2>
          ...
          {analytics.hours_by_month.length === 0 ? (
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={analytics.hours_by_month} margin={{ top: 8, right: 8, left: 0, bottom: 8 }}>
                  <defs>
                  ...
                  <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} />
                  <XAxis dataKey="month" tick={{ fontSize: 10, fill: '#64748b', fontWeight: 500 }} tickLine={false} axisLine={false} dy={6} />
                  <YAxis tick={{ fontSize: 10, fill: '#64748b', fontWeight: 500 }} tickLine={false} axisLine={false} dx={-4} width={32} />
                  <Tooltip cursor={{ fill: '#f8fafc', opacity: 0.8 }} />
      ...
      <Card>
        <h2 className="text-lg font-black text-slate-800 dark:text-slate-100 mb-1">Recent Activities</h2>
        <p className="text-xs font-semibold text-slate-400 mb-4">Latest verified volunteer activities from your team</p>
        {analytics.recent_activities.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-xs text-slate-300 dark:text-slate-600 mt-1">Activities will appear once team members complete verified gigs.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 dark:border-slate-700">
                  <th className="text-left py-3 px-2 text-xs font-black uppercase tracking-widest text-slate-400">Volunteer</th>
                  <th className="text-left py-3 px-2 text-xs font-black uppercase tracking-widest text-slate-400">Gig</th>
                  <th className="text-right py-3 px-2 text-xs font-black uppercase tracking-widest text-slate-400">Hours</th>
                  <th className="text-right py-3 px-2 text-xs font-black uppercase tracking-widest text-slate-400">Date</th>
                </tr>
              </thead>
              <tbody>
                {analytics.recent_activities.map((a, i) => (
                  <tr key={i} className="border-b border-slate-50 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="py-3 px-2 font-bold text-slate-700 dark:text-slate-200">{a.volunteer_name}</td>
                    ...
                    <td className="py-3 px-2 text-right text-slate-400 text-xs">{a.date?.slice(0, 10)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
